...
async def sync_company(db: Session, company: Company):
    ...
    try:
        raw_jobs = await scraper.scrape(company.board_token)
    ...
    seen_ids = []
    for j in raw_jobs:
        external_id = j["url"]  # Greenhouse/Lever URLs are stable per-posting IDs
        ...
        stmt = pg_insert(Job).values(
            ...
            set_={"title": j["title"], "location": j.get("location"),
                  "is_active": True, "scraped_at": func_now()},
        )
        db.execute(stmt)
